...
export default function StrategyBoardGrid({ data = rows }) {
  const navigate = useNavigate();
  return (
    <Box sx={{ width: '100%', maxWidth: { sm: '100%', md: '1700px' }, mx: 'auto', py: 4 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        <h1 style={{ margin: '0 16px 0 0' }}>전략 게시판</h1>
        <Button variant="outlined" size="small" onClick={() => navigate('/strategy-board/write')}>
          글쓰기
        </Button>
      </Box>
      <DataGrid
        ...
        onRowClick={(params) => navigate(`/strategy-board/${params.id}`)}
      />
      <Copyright sx={{ my: 4 }} />
    </Box>
  );
